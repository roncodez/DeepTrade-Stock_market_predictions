...
def insertintotable():
    ...
    def get_historical(quote):
        ...
        if(df.empty):
            ...
            ts = TimeSeries(key='GUUHGN4HUSL6UGNG',output_format='pandas')
    ...
    def retrieving_tweets_polarity(symbol):
        auth = tweepy.OAuthHandler('wpezKD1HRgAeDnBWUW8oa5s9b', 'IOinMRQSx8I4KF3AgGHBg7jlXxi1HxAKWG0FBAOGu7LsjZdW04')
        auth.set_access_token('1082648984-u5tsmjxVJkGLHamkyiWyOpMb0Abwv6RUqmPEn2L', 'UKJ9rts2uSfsPOLDMC8ZwhvHEaaAA862Orx9Vwb28fgIY')
        user = tweepy.API(auth)
        
        #tweets = tweepy.Cursor(user.search, q=str(symbol), tweet_mode='extended', lang='en',exclude_replies=True).items(int(300))
        tweets = tweepy.Cursor(user.search_tweets, q=str(symbol), tweet_mode='extended', lang='en', exclude_replies=True).items(int(300))
